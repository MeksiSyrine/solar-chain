...
async function main() {
  ...
  const producerProfile = await new ProducerProfile__factory(deployer).deploy();
  await producerProfile.waitForDeployment();
  ...
  const deployment = {
    ...
    contracts: {
      ...
      producerProfile: await producerProfile.getAddress()
    }
  };
  ...
  console.log(`ProducerProfile: ${deployment.contracts.producerProfile}`);
