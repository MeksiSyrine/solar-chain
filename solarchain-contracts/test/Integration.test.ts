...
describe("SolarChain Integration", function () {
  it("should run end-to-end admin -> producer -> consumer flow", async function () {
    ...
    expect(await token.balanceOf(producer.address)).to.equal(500n);
    ...
    expect(offer.remainingKwh).to.equal(180n);

    expect(await token.balanceOf(consumer.address)).to.equal(120n);
    ...
    expect(history[0].quantityKwh).to.equal(120n);
  });
  ...
  it("should support multiple readings and multiple consumers", async function () {
    ...
    expect(await token.balanceOf(consumer.address)).to.equal(100n);
    expect(await token.balanceOf(other.address)).to.equal(50n);

    const offer = await market.getOffer(1);
    expect(offer.remainingKwh).to.equal(350n);
